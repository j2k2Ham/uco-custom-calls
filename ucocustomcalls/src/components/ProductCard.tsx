...
export function ProductCard({ product, priority }: { readonly product: Product; priority?: boolean }) {
  return (
  <li className="rounded-lg overflow-hidden border border-camo-light hover:border-brass/60 transition flex bg-camo-light/30 w-full">
      <Link href={`/products/${product.slug}`} className="flex flex-col flex-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brass/60">
        <div className="relative aspect-square">
          <Image src={product.images[0].src} alt={product.images[0].alt} fill priority={priority} sizes="(min-width: 1024px) 33vw, 100vw" className="object-cover" />
        </div>
  <div className="p-4 flex flex-col flex-1 min-h-[180px]">
    <h3 className="font-medium flex-0 leading-snug">{product.title}</h3>
